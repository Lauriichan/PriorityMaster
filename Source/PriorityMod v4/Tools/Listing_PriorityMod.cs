...
namespace PriorityMod.Tools
{
	...
	{ 
		...
		private bool DrawPicker(float x, float width, float height, ref float hue, float saturation, ref float lightness)
		{
			...
			GUI.DrawTexture(new Rect(box.x + hue * width - selectionHalfSize, (1f - lightness) * height - selectionHalfSize, selectionSize, selectionSize), UIResources.SelectionCircle, ScaleMode.ScaleToFit, true, 1f, lightness > 0.25f ? Color.black : Color.white, 0f, 0f);
